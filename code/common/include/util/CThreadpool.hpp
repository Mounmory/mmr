...
//线程池类型
class COMMON_CLASS_API ThreadPool
{
private:
	//线程池构造
	ThreadPool();
...
public:
	...
	//设置task任务队列上线的阈值
	void setTaskQueMaxThrshHold(const uint32_t& threshhold);

	//设置线程池cached模式下线程阈值
	void setThreadSizeThreshHold(const uint16_t& threshHold);

	void setThreadSize(const uint16_t& treadSize);
